...
$sql = "select ID,Agname,language from web_world where Oid='$uid'";
...
while ($row1=mysql_fetch_array($result1)){
	$gid=$row1['mid'];
	$sql="select count,score,id,mshow2,gid,team_tw as team,rate from sp_match as t left join (select mid,count(*) as count,sum(betscore) as score from web_db_io where linetype=16 and world='$agname' and result_type=0 group by mid order by id) as s on t.id=s.mid where t.mid=$gid order by rate,id ASC";
	$result = mysql_query( $sql);
	$str_data='';
	$str_data1='';
