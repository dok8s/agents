...
$mysql="SELECT m_league as m_league FROM `bask_match` WHERE `m_start` > now( ) and `m_Date` ='$mDate' group by m_league";
$result = mysql_query( $mysql);
...
switch ($rtype){
case "all":
	$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,M_LetB as M_LetB_en,$m_league as M_League,MB_Dime_Rate,TG_Dime_Rate,if(m_dime='','',concat('o',m_dime)) as MB_Dime_en,MB_LetB_Rate,TG_LetB_Rate,MB_MID,TG_MID,ShowType,R_Show,if(s_single='','',concat('?',s_single)) as s_single,if(s_double='','',concat('?',s_double)) as s_double from bask_match where`m_start` > now( ) AND `m_Date` ='$mDate'".$sleague."  order by m_league desc,mid";
	...
	sum(if(linetype=3 and mtype='C',betscore,0)) as oucs from web_db_io where linetype in (5,2,3) and agents='$agname' and hidden=0 and m_date='$bdate' group by mid order by id";
...
case "re":
	...
	sum(if(linetype=10 and mtype='C',betscore,0)) as oucs from web_db_io where linetype in (5,9,10) and agents='$agname' and hidden=0 and m_date='$bdate' group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
...
case "par":
	...
	sum(if(linetype=8 and mtype='C',betscore,0)) as rcs from web_db_io where linetype=8 and agents='$agname' and hidden=0 and m_date='$bdate' group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
