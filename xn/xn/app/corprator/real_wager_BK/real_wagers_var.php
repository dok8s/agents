...
case "all":
	...
	sum(if(linetype=3 and mtype='C',betscore,0)) as oucs from web_db_io where linetype in (5,2,3) and corprator='$agname' and hidden=0 and m_date='$bdate' group by mid order by id";
...
case "re":
	...
	sum(if(linetype=10 and mtype='C',betscore,0)) as oucs from web_db_io where linetype in (5,9,10) and corprator='$agname' and hidden=0 and m_date='$bdate' group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
...
case "par":
	...
	sum(if(linetype=8 and mtype='C',betscore,0)) as rcs from web_db_io where linetype=8 and corprator='$agname' and hidden=0 and m_date='$bdate' group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
