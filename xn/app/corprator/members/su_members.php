...
$sql = "select Agname,ID,language,super,edit,setdata from `web_corprator` where Oid='$uid'";
$result = mysql_query($sql);
...
case 2:
	...
	mysql_query( $mysql);
	$mysql="insert into  agents_log (M_DateTime,M_czz,M_xm,M_user,M_jc,Status) values('".date("Y-m-d H:i:s")."','$agname','$xm','$agent_name','Thành viên',3)";
...
<FORM NAME="myFORM" ACTION="/xn/app/corprator/members/su_members.php?uid=<?=$uid?>" METHOD=POST style="padding-left:20px;padding-top:10px;">
...
if($enable=='Y'){
?>
<a href="javascript:CheckSTOP('/app/corprator/members/su_members.php?uid=<?=$uid?>&active=2&id=<?=$row['ID']?>&enable=S','S')">Tạm ngưng</a> /
<?
}
?><a href="javascript:CheckSTOP('/app/corprator/members/su_members.php?uid=<?=$uid?>&active=2&id=<?=$row['ID']?>&enable=<?=$memstop?>','<?=$memstop?>')"><?=$caption1?></a> / <a href="./su_mem_edit.php?uid=<?=$uid?>&mid=<?=$row['ID']?>">Sửa đổi dữ liệu</a> / <a href="su_mem_set.php?uid=<?=$uid?>&id=<?=$row['ID']?>&pay_type=<?=$row['pay_type']?>&agents_id=<?=$row['Agents']?>">Cài đặt chi tiết</a></td>
    </tr>
<?
}
}
