...
$sql = "select Agname from `web_corprator` where Agname ='$username' and Passwd='$password' and passwd_safe=''";
...
$sql 			= "select id,if(time_to_sec(timediff(now(),if(logintime='0000-00-00 00:00:00','2007-12-01 12:00:00',logintime)))>180,0,1) as timeout from `web_corprator` where Agname ='$username' and Passwd='$password' and passwd_safe='$passwd_safe' and (Status=1 or Status=2)";
$result 	= mysql_query($sql);
...
if ($count==0){
	...
	//if ($row['timeout']==0){
		...
		$sql="update web_corprator set oid='$uid',logintime=now(),domain='$_SERVER[HTTP_HOST]',language='zh-cn',logip='$ip_addr' where Agname='".$username."'";
		mysql_query($sql) or die ("操作失败!");
        	$tu=rand(13455150,83459150);
             $_SESSION["akak"]=$tu;
		$loginfo='用户登陆成功';
		$mysql="insert into web_mem_log(username,logtime,context,logip,level) values('$username',now(),'$loginfo','$ip_addr','1')";
		mysql_query($mysql);
		show_message('d1:'.$username);

	echo '<script>
top.user_id = \''.$uid.'\';
top.flash_cont_cutoff = \'Y\';top.flash_cont_flag = \'300\';top.langx = \'zh-cn\';
</script>
<html>
<head>
<title>登入1</title>
