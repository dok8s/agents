...
<FORM NAME="myFORM" ACTION="/app/corprator/members/su_members.php?uid=<?=$uid?>" METHOD=POST style="padding-left:20px;padding-top:10px;">
...
if($enable=='Y'){
?>
<a href="javascript:CheckSTOP('/app/corprator/members/su_members.php?uid=<?=$uid?>&active=2&id=<?=$row['ID']?>&enable=S','S')">暂停</a> /
<?
}
?><a href="javascript:CheckSTOP('/app/corprator/members/su_members.php?uid=<?=$uid?>&active=2&id=<?=$row['ID']?>&enable=<?=$memstop?>','<?=$memstop?>')"><?=$caption1?></a> / <a href="./su_mem_edit.php?uid=<?=$uid?>&mid=<?=$row['ID']?>">修改资料</a> / <a href="su_mem_set.php?uid=<?=$uid?>&id=<?=$row['ID']?>&pay_type=<?=$row['pay_type']?>&agents_id=<?=$row['Agents']?>">详细设定</a></td>
    </tr>
<?
}
}
