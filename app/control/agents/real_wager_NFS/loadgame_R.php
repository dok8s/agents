...
function layer_screen(gid,layers){
	...
	//changeRatio[] [1=变动] 0:R_con  1:RH_Ratio 2:RH_Ratio 3:OU_con  4:OUH_Ratio 2:OUC_Ratio

		gno=gidx[gid];
		//GameFT[gno][4]=GameFT[gno][4].replace("[H]",top.str_home).replace("[M]",top.str_mid);
		layers=layers.replace("*GID*",GameFT[gno][0]);/*gid*/
		layers=layers.replace("*GID*",GameFT[gno][0]);/*gid*/
		layers=layers.replace("*TIME*",change_time(GameFT[gno][1]));/*时间*/

		layers=layers.replace("*LEG*",GameFT[gno][2]);/*联盟*/
		layers=layers.replace("*ITEM*",GameFT[gno][3]); /*场次*/
		//layers=layers.replace("*TEAM_H*",GameFT[gno][4]);/*主队*/
		//layers=layers.replace("*TEAM_C*",GameFT[gno][5]);/*客队*/
		...
		if (GameFT[gno][4]=="N"){
			...
			TBG='bgcolor="#cccccc"';
			layers=layers.replace("*GOPEN*","<font color='blue' style='cursor:hand;' onclick=\"if (check_fl("+gid+",'6')) return;change_game('gopen','Y',"+gid+",'"+se+"');\">"+top.str_gopen+"</font>");/*开放*/
		}else{
			layers=layers.replace("*CLASS*",'bgcolor=#ffffff');
			TBG='bgcolor="#ffffff"';
			layers=layers.replace("*GOPEN*","<font color='blue' style='cursor:hand;' onclick=\"if (check_fl("+gid+",'6')) return;change_game('gopen','N',"+gid+",'"+se+"');\">"+top.str_gameclose+"</font>");/*关闭*/
		}
		if (GameFT[gno][5]*1 > 0){
			if (GameFT[gno][6]=="0"){
				layers=layers.replace("*FL_SET*"," | <font style='cursor:hand;' color='red' onclick=change_game('FL_enable','1',"+gid+",'"+se+"')>"+top.str_FL+"</font>");/*跟盘*/
			}else{
		...
		//layers=layers.replace("*RESULT*","<font color='blue' style='cursor:hand;' onclick=\"chk_result('"+gno+"');\">"+top.str_result+"</font>");/*开放*/
...
str_close=top.str_close_ioratio;/*是否确定关闭赔率*/
a=confirm(str_close);
	if (a==true){
		grtypes="";
		gratios="0";
...
function chkCurrentDate(val){
	...
	if(month == 4 || month == 6 || month == 9 || month == 11) {
		...
		if(year == 0 && day > 29) return  false;  // 润年
...
str_FL_enable1="确定改变跟盘状态";
str_FL_enable0="确定改变跟盘状态";
...
<td><input type="button" value="确定" onClick="chgDate();"></td>
          </tr>
      </table>
 </td>
...
<table height="0" cellpadding="1" cellspacing="1">
  <tr>
<td><font color="#000099">&nbsp;区域:</font></td><td><span id="sel_areas"></span></td>

<td>&nbsp;&nbsp;选择联盟 :</td><td><span id="sel_leg"></span></td>

<td>&nbsp;&nbsp;选择类别 :</td><td><span id="sel_items"></span></td>
	  ...
	  <tr>
	    <td bgcolor="#FFFFFF">
	      <table width="220" border="0" cellspacing="0" cellpadding="0" class="m_tab_fix">
	          <tr bgcolor="#0163A2">
	          <td width="200" id=bet_title><font color="#FFFFFF">请设定关闭时间</font></td>
