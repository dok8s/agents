...
case 'OU':
	...
	$sql="SELECT m_league as m_league FROM `baseball` WHERE r_show=3 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'HOU':
	$mysql="SELECT * FROM `baseball` WHERE mid%2=0 and r_show=2 and `m_start` > now( ) and `m_Date` ='$mDate'";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE r_show=2 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'RE':
	$mysql="SELECT * FROM `baseball` WHERE re_show=1 and `m_start` > now( ) and `m_Date` ='$mDate'";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE re_show=1 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'PD':
	$mysql="SELECT * FROM `baseball` WHERE pd_show=1 and `m_start` > now( ) and `m_Date` ='$mDate'";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE pd_show=1 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'HPD':
	$mysql="SELECT * FROM `baseball` WHERE mid%2=0 and pd_show=2 and `m_start` > now( ) and `m_Date` ='$mDate'";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE pd_show=2 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'F':
	$mysql="SELECT * FROM `baseball` WHERE f_show=1 and `m_start` > now( ) and `m_Date` ='$mDate'";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE f_show=1 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'T':
	$mysql="SELECT * FROM `baseball` WHERE t_show=1 and `m_start` > now( ) and `m_Date` ='$mDate'";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE t_show=1 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'P':
	$mysql="SELECT * FROM `baseball` WHERE p_show=1 and `m_start` > now( ) and `m_Date` ='$mDate'";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE p_show=1 and `m_start` > now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
case 'PL':
	$mysql="SELECT * FROM `baseball` WHERE mid%2=1 and `m_Date` ='$mDate' and m_start<now()";
	$sql="SELECT m_league as m_league FROM `baseball` WHERE r_show=1 and `m_start` < now( ) and `m_Date` ='$mDate' group by m_league order by gid";
	break;
}
...
case "RE":
	...
	break;
case "PD":
	$sql1 = "select mid,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league as league,ShowType,if(PD_Show=1,'Y','N') as pd_show from baseball where pd_show=1 and mid%2=1 and `m_start` > now( ) AND `m_Date` ='$mDate' ".$sleague." order by m_start,mid limit $offset,40";
	$sql2	=	"select mid as mid2,count(*) as cou,sum(BetScore) as score FROM `web_db_io` where linetype=4 and agents='$agname' and hidden=0  and m_date='$bdate' group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
...
case "HPD":
	...
	$sql2	=	"select mid as mid2,count(*) as cou,sum(BetScore) as score FROM `web_db_io` where linetype=34 and agents='$agname' and hidden=0 and m_date='$bdate'  group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
...
case "F":
	...
	$sql2	=	"select mid as mid2,count(*) as cou,sum(BetScore) as score FROM `web_db_io` where linetype=14 and agents='$agname' and hidden=0 and m_date='$bdate'  group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
...
case "P":
	...
	//$sql1 = "select mid,concat(M_Date,'<br>',M_Time,if(m_type=0,'','<br><font style=background-color=red>ǐ</font>')) as pdate,$mb_team as MB_Team,$tg_team as TG_Team,m_letb as M_LetB_en,m_league as league,MB_Win,TG_Win,M_Flat,MB_Dime_Rate,TG_Dime_Rate,if(m_dime='','',concat('o',m_dime)) as MB_Dime_en,MB_LetB_Rate,TG_LetB_Rate,MB_MID,TG_MID,ShowType,M_Type,R_Show from baseball where r_show=1 and `m_Date` ='$mDate' and `m_start` > now( ) ".$sleague."  order by display,mid limit $offset,40";
	...
	sum(if(mtype='N',betscore,0)) as mns from web_db_io where linetype in (7,8) and agents='$agname' and hidden=0 and m_date='$bdate' group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
